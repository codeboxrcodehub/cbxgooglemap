;(function ($) {
  ...
  function cbxgooglemap_isEmptyOrUndefined(val) {
    return val === undefined || val === null || val === '';
  }//end method cbxgooglemap_isEmptyOrUndefined

  $(document).ready(function () {
    var cbxgooglemap_setting_awn_options = {
      labels: {
        tip          : cbxgooglemap_setting_js_var.awn_options.tip,
        info         : cbxgooglemap_setting_js_var.awn_options.info,
        success      : cbxgooglemap_setting_js_var.awn_options.success,
        warning      : cbxgooglemap_setting_js_var.awn_options.warning,
        alert        : cbxgooglemap_setting_js_var.awn_options.alert,
        async        : cbxgooglemap_setting_js_var.awn_options.async,
        confirm      : cbxgooglemap_setting_js_var.awn_options.confirm,
        confirmOk    : cbxgooglemap_setting_js_var.awn_options.confirmOk,
        confirmCancel: cbxgooglemap_setting_js_var.awn_options.confirmCancel
      }
    };
    ...
    $('.setting-color-picker-wrapper').each(function (index, element) {
      ...
      const pickr = Pickr.create({
        ...
        i18n      : cbxgooglemap_setting_js_var.pickr_i18n
      });
    ...
    $('.selecttwo-select-wrapper').each(function (index, element) {
      ...
      if(cbxgooglemap_isEmptyOrUndefined($placeholder)) $placeholder = cbxgooglemap_setting_js_var.please_select;
      ...
      $element
          ...
          .on('select2:open', function () {
            $('.select2-search__field').attr(
                'placeholder',
                cbxgooglemap_setting_js_var.placeholder.search
            );
          })
    ...
    var $setting_page = $('#cbxgooglemap-setting');
    ...
    function setting_nav_change($tab_id) {
      ...
      if (clicked_group === "#cbxgooglemap_tools") {
        $.ajax({
          type: 'POST',
          dataType: 'json',
          url: cbxgooglemap_setting_js_var.ajax.ajax_url,
          data: {
            action: "cbxgooglemap_settings_reset_load",
            security: cbxgooglemap_setting_js_var.ajax.ajax_nonce,
          },
      ...
      if (typeof (localStorage) !== 'undefined') {
        localStorage.setItem('cbxgooglemapactivetab', clicked_group);
      }
    ...
    $("#setting_resetinfo").on(
      'click',
      ".cbxgooglemap_setting_options_check_action_call",
      function (e) {
        e.preventDefault();
    ...
    $("#setting_resetinfo").on(
      'click',
      ".cbxgooglemap_setting_options_check_action_ucall",
      function (e) {
        e.preventDefault();
    ...
    $('#reset_data_trigger').on('click', function (e) {
      ...
      var notifier = new AWN(cbxgooglemap_setting_awn_options);
      ...
      $.ajax({
        ...
        url: cbxgooglemap_setting_js_var.ajax.ajax_url,
        data: {
          action: 'cbxgooglemap_settings_reset',
          security: cbxgooglemap_setting_js_var.ajax.ajax_nonce,
      ...
      notifier.confirm(
          cbxgooglemap_setting_js_var.delete_dialog.are_you_sure_delete_desc,
          onOk,
          onCancel,
          {
            labels: {
              confirm: cbxgooglemap_setting_js_var.delete_dialog.are_you_sure_global
            }
          }
      );
    ...
    $('.clear-permalink').on('click', function (e) {
      ...
      $.ajax({
        ...
        url: cbxgooglemap_setting_js_var.ajax.ajax_url,
        data: {
          action: 'cbxgooglemap_permalink_cache_clear',
          security: cbxgooglemap_setting_js_var.ajax.ajax_nonce,
        },
        success: function (response, textStatus, XMLHttpRequest) {
          let notifier = new AWN(cbxgooglemap_setting_awn_options);
          if (response.success) {
    ...
    $('#cbxgooglemap-setting').on('click', '#cbx-export-import', function (e) {
      e.preventDefault();
      $('.cbx-export-import-wrapper').slideToggle('fast');
    });

    // click to open changelog modal
    var import_modal_notifier = null;

    //on click import changelog setiing as json
    $('#cbxgooglemap_settings_import').on('click', function (e) {
      e.preventDefault();

      import_modal_notifier = new AWN().modal(
          cbxgooglemap_setting_js_var.import_modal,
          'changelog_import_modal',
          cbxgooglemap_setting_awn_options
      );
    });

    const convertBase64_settings_json = (file) => {
    ...
    //import settings
    const upload_cbxgooglemap_settings_json_file = async (event) => {
      const file = event.target.files[0];
      const base64 = await convertBase64_settings_json(file);


      $('#cbxgooglemap_settings_import_form').remove();
      $('#cbxgooglemap_settings_import_modal_wrap').append(cbxgooglemap_setting_js_var.import_modal_progress);
      ...
      $.ajax({
        ...
        url: cbxgooglemap_setting_js_var.ajax.ajax_url,
        data: {
          action: 'cbxgooglemap_settings_import',
          security: cbxgooglemap_setting_js_var.ajax.ajax_nonce,
          settings_data: base64,
        },
        success: function (data, textStatus, XMLHttpRequest) {
          //$this.hide();

          import_modal_notifier.delete();

          if (data.success) {
            new AWN(cbxgooglemap_setting_awn_options).success(data.message);

            window.location.href = data.url;
          } else {
            new AWN(cbxgooglemap_setting_awn_options).alert(data.message);
          }
    ...
    $(document.body).on("change", '#cbxgooglemap_settings_import_file', (e) => {
      upload_cbxgooglemap_settings_json_file(e);
    });

    //reset single section
    $('.submit_reset_section').on('click', function (e){
      e.preventDefault();
      var $this = $(this);
      ...
      var onOk = () => {
        ...
        $.ajax({
          ...
          url: cbxgooglemap_setting_js_var.ajax.ajax_url,
          data: {
            action: 'cbxgooglemap_settings_reset_section',
            security: cbxgooglemap_setting_js_var.ajax.ajax_nonce,
            section: $section
          },
          success: function (data, textStatus, XMLHttpRequest) {
      ...
      notifier.confirm(
          cbxgooglemap_setting_js_var.are_you_sure_delete_desc,
          onOk,
          onCancel,
          {
            labels: {
              confirm: cbxgooglemap_setting_js_var.are_you_sure_global
            }
          }
      );
    });
