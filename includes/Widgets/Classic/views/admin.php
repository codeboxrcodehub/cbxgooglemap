...
?>
    ...
    <p>
        ...
        <select class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'map_id' ) ); ?>"
            ...
            <?php
            foreach ( $cbxgooglemap_query as $post ) :
                CBXGooglemapHelper::setup_admin_postdata( $post );
                $post_id                 = intval( get_the_ID() );
                $cbxgooglemap_post_title = get_the_title();

                echo '<option ' . selected( $post_id, $map_id, false ) . ' value="' . absint( $post_id ) . '">' . esc_attr( $cbxgooglemap_post_title ) . '</option>';
