...
final class CBXGoogleMapAdmin {
		...
		$settings_builtin_fields = [
			'cbxgooglemap_general' => [
				...
				'width'           => [
					...
					'desc'    => esc_html__( 'Default is 100% to make the map responsive, if you want any fixed width then don\'t put, % or just put numeric value, don\'t px with numeric value',
						'cbxgooglemap' ),
					'type'    => 'text',
					'default' => '100%',
			...
			'cbxgooglemap_tools'   => [
				...
				'delete_global_config' => [
					...
					'desc'    => '<p>' . esc_html__( 'Delete Global Config data(options/plugin settings), custom table(s), files/folders, all map custom post type created by this plugin on uninstall. Please note that this process can not be undone and it is recommended to keep full database and files backup before doing this.',
							'cbxgooglemap' ) . '</p>',
					'type'    => 'radio',
					'options' => [
	...
	public function menu_pages() {
		//setting page
		$setting_page_hook = add_submenu_page( 'edit.php?post_type=cbxgooglemap', esc_html__( 'CBX Map: Settings', 'cbxgooglemap' ), esc_html__( 'Settings', 'cbxgooglemap' ), 'manage_options',
			'cbxgooglemap_settings', [
				$this,
				'menu_page_settings'
			] );

		$doc_page_hook = add_submenu_page( 'edit.php?post_type=cbxgooglemap', esc_html__( 'CBX Map Helps & Updates', 'cbxgooglemap' ), esc_html__( 'Helps & Updates', 'cbxgooglemap' ),
			'manage_options', 'cbxgooglemap_support', [
				$this,
				'menu_page_docs'
			] );
	}//end menu_pages
	...
	public function cbxgooglemap_manage_columns( $column_name ) {
		...
		$zoom = ( isset( $meta_combined[ $meta_prefix . 'zoom' ] ) && absint( $meta_combined[ $meta_prefix . 'zoom' ] ) > 0 ) ? absint( $meta_combined[ $meta_prefix . 'zoom' ] ) : '';
	...
	public function enqueue_scripts( $hook ) {
		...
		if ( ( $hook == 'post.php' || $hook == 'post-new.php' ) && $post_type == 'cbxgooglemap' ) {
			...
			$edit_js_vars = [
				...
				'api_key'               => $api_key,
				'no_api_key'            => ( $api_key === '' ) ? esc_html__( 'No google map api key found, please check CBX Map plugin setting.', 'cbxgooglemap' ) : '',
		...
		if ( $current_page == 'cbxgooglemap_settings' ) {
			...
			$setting_js_vars = [
				'global_setting_link_html' => '<a href="' . esc_url( admin_url( 'edit.php?post_type=cbxgooglemap&page=cbxgooglemap_settings' ) ) . '"  class="button outline primary pull-right">' . esc_html__( 'Global Settings',
						'cbxgooglemap' ) . '</a>',
		...
		if ( ( $hook == 'edit-tags.php' || $hook == 'term.php' ) && $post_type == 'cbxgooglemap' && $current_page == '' ) {
			...
			$global_setting_link_html = '<a href="' . esc_url( admin_url( 'edit.php?post_type=cbxgooglemap&page=cbxgooglemap_settings' ) ) . '"  class="button outline primary">' . esc_html__( 'Global Settings',
					'cbxgooglemap' ) . '</a>';
	...
	public function gutenberg_blocks() {
		...
		wp_register_style( 'cbxgooglemap-block', plugin_dir_url( __FILE__ ) . '../assets/css/cbxgooglemap-block.css', [],
			filemtime( plugin_dir_path( __FILE__ ) . '../assets/css/cbxgooglemap-block.css' ) );
